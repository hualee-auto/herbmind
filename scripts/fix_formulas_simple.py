...
# 测试第一个方剂
test_formula = formulas[0]
print(f"\nTest: {test_formula['id']}")

url = f"{BASE_URL}detail.php?lang={LANG}&id={test_formula['id']}"
...
with urllib.request.urlopen(req, timeout=30) as response:
    ...
    # 提取方名 - 【中文】
    name = extract_text(html, r'【中文】\s*([^【<]+)')
    print(f"  Name: {name}")

    # 提取拼音 - 【汉语】
    pinyin = extract_text(html, r'【汉语】\s*([^【<]+)')
    print(f"  Pinyin: {pinyin}")

    # 提取英文名 - 【英文】
    english = extract_text(html, r'【英文】\s*([^【<]+)')
    print(f"  English: {english}")

    # 提取类别 - 【分类 】
    category = extract_text(html, r'【分类\s*】\s*([^【<]+)')
    print(f"  Category: {category}")

    # 提取组成 - 【组成】
    comp_match = re.search(r'【组成】\s*([^【]+?)(?=【|$)', html, re.DOTALL)
    if comp_match:
        composition = comp_match.group(1).strip()
        composition = re.sub(r'<[^>]+>', '', composition)
        composition = ' '.join(composition.split())
